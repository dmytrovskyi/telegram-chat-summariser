...
#### Web Page Content Summary (if applicable) ####
- **URL 1 Summary**: A guide on stress management techniques that outlines methods like meditation, exercise, and time management.
- **URL 2 Summary**: A blog post about the benefits of hiking and spending time in nature for mental health.
- [https://github.com/infiniflow/infinity](https://github.com/infiniflow/infinity) - The Telegram Chat Summariser is a bot that summarizes recent messages in a chat using AI-powered language models (LLMs) and the OpenAI API. Since Telegram bots can't access chat history directly, messages are stored in a Firestore database. Users need to set up a Firestore database, configure a new bot via BotFather, and generate an OpenAI API key.
...
def ai_summarize_basic(messages, template):
...
def ai_summarize(messages):
    return ai_summarize_basic(messages, precise_template)
